"""VP viewpoint-robustness eval for BEVDet (3D-detection NDS) -- the BEVDet
analogue of eval_vp_robustness_det.py (BEVFormer) / _bevdepth.py.

A sedan-trained BEVDet model is evaluated on carla_VR viewpoint perturbations:
...
Mirrors the BEVFormer driver's optimisations so 631 cells stay tractable:
  ...
  * inference + eval go through BEVDet's OWN dataloader (PrepareImageInputs builds
...
DATA_ROOT = osp.join(BEVDET, 'data', 'nuscenes')   # images + per-vehicle eval DBs
...
def main():
    ...
    config = args.config if osp.isabs(args.config) else osp.join(BEVF_ROOT, args.config)
