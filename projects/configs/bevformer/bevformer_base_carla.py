"""BEVFormer-base adapted for CARLA jeongtae dataset.

Diffs from bevformer_base.py:
  * temporal off:  queue_length=1, video_test_mode=False, rotate_prev_bev=False,
                   use_can_bus=False
  ...
  * per-vehicle (viewpoint) data under data/nuscenes/ (-> carla_geobev). This is
    the SEDAN config; vehicle='sedan'. suv/bus variants: bevformer_base_carla_{suv,bus}.py
  * ann_file = {vehicle}_infos_train.pkl (v1.0-carla_{veh}),
               {vehicle}_infos_val.pkl   (v1.0-carla_{veh}_eval)
  * use_valid_flag=True (converter sets valid_flag from visibility>='2')

Original bevformer_base.py is NOT modified.
"""
_base_ = [
...
# Coordinate frame is identical to nuscenes (verified) -> keep range
...
bev_h_ = 200
bev_w_ = 200
queue_length = 2  # min size that lets forward_train run; we force prev_bev_exists=False
                  # in the converter (info['scene_token']=info['token']) so prev_bev is never used.
...
data = dict(
    samples_per_gpu=4,   # effective batch 8 across 2 GPUs = same as original 8-GPU x bs=1 setup
                         # B200 memory: ~96GB / 180GB used (safe)
    workers_per_gpu=16,  # 2 GPUs x 16 = 32 workers on 72-core EPYC (room to spare)
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ...
        test_mode=False,
        use_valid_flag=True,   # converter sets it from visibility>='2'
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
             init_kwargs=dict(
                 ...
                 notes='BEVFormer-base on CARLA jeongtae. queue_length=2 + '
                       'unique scene_token (temporal disabled). 6 classes. '
                       'visibility>=2 valid_flag. samples_per_gpu=4 x 2 B200 '
                       '= global batch 8 (matches original 8-GPU x bs1 setup).'),
             interval=50,
             commit=True,
             by_epoch=True,
             with_step=True,
