...
class BEVFormerSeg(BEVFormer):
    ...
    def forward_train(self, img_metas=None, img=None, gt_seg=None, **kwargs):
        ...
        # gt_seg is already visibility-filtered (3DOD-style: low-vis vehicles are
        # background), so the loss runs over the full grid like the IoU metric.
        gt = gt_seg.float()
        if gt.dim() == 3:
            gt = gt[:, None]
